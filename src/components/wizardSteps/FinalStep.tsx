...
const FinalStep = ({ onBeforeSend, onError} : FinalStepProps) => {
  const { getData, inputGroups, setResult, form: { Disclaimer, CalculatorVersion } } = useSimulationStore((state) => state);
    ...
    <Grid container spacing={2} style={{height: '620px'}}>
      ...
      <Grid size={{ xs: 12, md: 12 }} container justifyContent='center' alignContent='flex-start'>
        <Tooltip title='Exportar dados preenchidos para continuar mais tarde' arrow>
          <Button variant="outlined" onClick={()=>exportToJSONFile(CalculatorVersion, inputGroups)} endIcon={<FileDownload />} >Exportar</Button>
        </Tooltip>
